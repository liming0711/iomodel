...
const UnalignedPanel: React.FC<{ data: OperationOptions | undefined }> = (props) => {
  const { data } = props;
  const [urSeries, setUrSeries] = useState<any>([
  ...
  const [urAxis, setUrAxis] = useState<any[]>([]);
  const [urTitle, setUrTitle] = useState<any[]>([]);
  const [urColumns, setUrColumns] = useState<ColumnProps[]>([]);
  const [urDataSource, setUrDataSource] = useState<any[]>([]);
  ...
  useEffect(() => {
    ...
    setUrSeries(_opData);
    setUrColumns(_opColumns);
    setUrDataSource(_opDataSource);
  }, [data]);

  return (
    <div className="io-container-item">
      <Panel title="重叠读分析（图表）">
        <ScatterChart data={urSeries} />
      </Panel>
      <Panel title="重叠读分析（数据）">
        <Table
          dataSource={urDataSource}
          columns={urColumns}
          size="small"
          pagination={{
