  ...
  useEffect(() => {
    ...
    keys.forEach((key) => {
      ...
      files.forEach((filename, idx) => {
        ...
        aa[key]['read'][filename].forEach((item) => {
          ...
          let index = columns.findIndex((v) => v.key === key);
          if (!index) {
            columns.push({
              key,
              title: key,
              children: [
                { key: `${key}_offset`, title: 'Offset', dataIndex: `${key}_offset` },
                { key: `${key}_length`, title: 'Length', dataIndex: `${key}_length` },
                { key: `${key}_count`, title: 'Count', dataIndex: `${key}_count` },
              ]
            });
            (seriesItem.data as [number, number][]).push([columns.length - 1, item[1]]);
          } else {
            (seriesItem.data as [number, number][]).push([index, item[1]]);
          }
          dataItem[`${key}_offset`] = item[0][0];
          dataItem[`${key}_length`] = item[0][1];
          dataItem[`${key}_count`] = item[1];
        });
    ...
    title.forEach((item, idx) => {
      const axisItem = {
        ...
        boundaryGap: false,
        data: [],
        top: (idx * 100) / (files.length + 1) + 8 + '%',
        height: 100 / (files.length + 1) - 5 + '%'
      };
      const seriesItem = {
        singleAxisIndex: idx,
      ...
      (axisItem.data as string[]).push(key);
      singleAxis.push(axisItem);
      columns.forEach((c) => {
