...
const IO: React.FC = () => {
  ...
  useEffect(() => {
    if (selected && selected.length >= 2) {
      ...
      setLoading(true);
      getInfo(selected[0], selected[1])
        .then((res) => {
          console.log('--- Info ---', res);
          const { steps = [] } = res;
          setSteps(steps);
          setInfo([res]);
          getAnalysedData(selected[0], 'file_op_count', steps[0], selected[1], selected[2])
            .then((res) => {
          ...
          getAnalysedData(selected[0], 'read_write', steps[0], selected[1], selected[2])
            .then((res) => {
          ...
          getAnalysedData(selected[0], 'parabuffer', steps[1], selected[0])
            .then((res) => {
          ...
          .then((ranges) => {
            ...
            axios.all<any>(requests2)
              .then((infos) => {
                ...
                setMenuData(_menus);
                setSelected(handleSelectedPath(_menus));
              })
              .catch((err) => {
                console.log(err);
              });
  ...
  const onSelectedChange = (_selected: string[]) => {
